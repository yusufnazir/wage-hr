...
export default function PlatformBankTemplatesPage() {
  const { me } = useTenantAppSession();
  const t = useCallback((key: string) => navLabel(me.locale, key), [me.locale]);

  const [load, setLoad] = useState<LoadState>("loading");
  const [items, setItems] = useState<PlatformBankTemplateRow[]>([]);
  ...
  const [confirm, setConfirm] = useState<
    { kind: "activate" | "deactivate"; row: PlatformBankTemplateRow } | null
  >(null);
  ...
  const reload = useCallback(
    async (p: number) => {
      setMsg(null);
      const r = await fetchPlatformBankTemplates({
        page: p,
        size: 20,
  ...
  async function runConfirmed() {
    ...
    try {
      if (confirm.kind === "deactivate") {
        await patchDeactivatePlatformBankTemplate(row.id);
      } else if (confirm.kind === "activate") {
        await patchActivatePlatformBankTemplate(row.id);
      }
      setConfirm(null);
      await reload(page);
    } catch {
      setMsg(t("platformBankTemplates.error.action"));
    } finally {
      setBusyId(null);
    }
  }
  ...
  if (!me.platformSuperadmin) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("platformBankTemplates.title")}</h1>
  ...
  if (load === "forbidden") {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("platformBankTemplates.title")}</h1>
        <p className="text-sm text-muted">{t("platformBankTemplates.error.forbidden")}</p>
        <Link href="/app" className="text-sm font-medium text-primary underline-offset-4 hover:underline">
          {"\u2190 "}
          {t("nav.dashboard")}
        </Link>
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
          ...
          <div className="max-w-md rounded-lg border border-border bg-surface p-5 shadow-lg">
            <p className="text-sm text-foreground">
              {confirm.kind === "deactivate"
                ? t("platformBankTemplates.confirm.deactivate")
                    .replace("{name}", confirm.row.name)
                    .replace("{country}", confirm.row.countryCode)
                : t("platformBankTemplates.confirm.activate").replace("{name}", confirm.row.name)}
            </p>
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Link
            ...
            className="rounded bg-primary px-3 py-1.5 text-sm font-semibold text-primary-foreground"
          >
            + {t("platformBankTemplates.action.new")}
          </Link>
          <Link href="/app" className="text-sm font-medium text-primary underline-offset-4 hover:underline">
            {"\u2190 "}
            {t("nav.dashboard")}
      ...
      <p className="text-sm text-muted">{t("platformBankTemplates.helper.intro")}</p>
      ...
      <section className="rounded-lg border border-border bg-surface/80 p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-5 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between sm:gap-x-10 sm:gap-y-4">
          <div className="flex min-w-0 flex-1 flex-col gap-2 sm:max-w-md">
            <label className="text-xs font-medium uppercase tracking-wide text-muted" htmlFor="platform-bank-templates-country">
              {t("platformBankTemplates.label.country")}
            </label>
            <select
              id="platform-bank-templates-country"
            ...
            >
              <option value="">{t("platformBankTemplates.filter.country.all")}</option>
              {countries.map((c) => (
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2 border-t border-border pt-4 sm:border-l sm:border-t-0 sm:pl-8 sm:pt-0">
            ...
            >
              {t("platformBankTemplates.filter.active.all")}
            </button>
            <button
              type="button"
            ...
            >
              {t("platformBankTemplates.filter.active.active")}
            </button>
          </div>
        </div>
      </section>
      ...
      {load === "error" ? (
        <p className="text-sm text-muted">{t("platformBankTemplates.error.load")}</p>
      ) : (
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-sm">
            <thead className="bg-surface text-left text-xs font-medium uppercase text-muted">
              <tr>
                <th className="px-3 py-2">{t("platformBankTemplates.col.country")}</th>
              ...
              {items.map((row) => (
                <tr key={row.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-right">
                    <Link
                      href={`/app/platform-bank-templates/${row.id}/edit`}
                      className="mr-2 text-primary underline-offset-4 hover:underline"
                    >
                      {t("platformBankTemplates.action.edit")}
                    </Link>
                    {row.active ? (
                      <button
                        type="button"
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        >
          {t("platformBankTemplates.action.prev")}
        </button>
        <span className="text-muted">
          {t("platformBankTemplates.pageIndicator").replace("{n}", String(page + 1)).replace("{t}", String(totalPages))}
        </span>
        <button
          type="button"
          className="rounded border border-border px-3 py-1 disabled:opacity-40"
        ...
        >
          {t("platformBankTemplates.action.next")}
        </button>
      </div>
    </div>
  );
