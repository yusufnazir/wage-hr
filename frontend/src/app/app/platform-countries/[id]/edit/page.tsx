...
import { fetchPlatformCountry, putPlatformCountry } from "@/lib/api";
import { navLabel } from "@/messages/nav";

type LoadState = "loading" | "ready" | "forbidden" | "notFound" | "error";

export default function PlatformCountryEditPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const r = await fetchPlatformCountry(id, me.locale);
      if (!r.ok) {
        setLoad(r.status === 403 ? "forbidden" : r.status === 404 ? "notFound" : "error");
        return;
      }
      setIsoAlpha2(r.item.isoAlpha2);
      setIsoAlpha3(r.item.isoAlpha3);
  ...
  if (!me.platformSuperadmin) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("platformCountries.title.edit")}</h1>
  ...
  if (load !== "ready") {
    const key =
      load === "forbidden"
        ? "platformCountries.error.forbidden"
        : load === "notFound"
          ? "platformCountries.error.notFound"
          : "platformCountries.error.load";
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("platformCountries.title.edit")}</h1>
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(t("platformCountries.msg.saveFailed"));
      setBusy(false);
    }
  }

  return (
    <div className="mx-auto max-w-xl space-y-6" data-testid="platform-country-form-edit">
      ...
      <form onSubmit={(e) => void onSubmit(e)} className="space-y-4 rounded-md border border-border bg-surface p-5 shadow-sm">
        ...
        <div className="flex gap-3">
          <button
            ...
            data-testid="platform-country-save"
          >
            {t("platformCountries.action.save")}
          </button>
          <Link
