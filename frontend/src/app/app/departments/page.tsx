...
type ModalMode = { kind: "create" } | { kind: "edit"; item: TenantDepartmentItem } | null;

function emptyPayload(companyId = ""): TenantDepartmentUpsertPayload {
  return { companyId, name: "", code: "", description: "", active: true };
}

function itemToPayload(item: TenantDepartmentItem): TenantDepartmentUpsertPayload {
  return {
...
export default function DepartmentsPage() {
  ...
  function openEdit(item: TenantDepartmentItem) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (modal?.kind === "create") {
        await createTenantDepartment(payload);
        setMsg(t("departments.msg.created"));
      } else if (modal?.kind === "edit") {
        await putTenantDepartment(modal.item.id, payload);
        setMsg(t("departments.msg.saved"));
      }
      closeModal();
      await reload(page);
    } catch (e) {
      setFormMsg(modal?.kind === "create" ? t("departments.msg.createFailed") : t("departments.msg.saveFailed"));
      console.error(e);
    } finally {
      setFormBusy(false);
    }
  ...
  async function toggleActive(item: TenantDepartmentItem) {
    ...
    setMsg(null);
    try {
      await patchTenantDepartmentActive(item.id, !item.active);
      await reload(page);
    } catch {
      setMsg(t("departments.msg.saveFailed"));
    } finally {
      setBusyId(null);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6" data-testid="departments-page">
      ...
      {load === "loading" && <p className="text-sm text-muted">{t("departments.state.loading")}</p>}
      {load === "error" && <p className="text-sm text-destructive">{t("departments.error.load")}</p>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-lg border border-border bg-surface p-6 shadow-xl">
            <h2 className="mb-4 text-base font-semibold text-foreground">
              {modal.kind === "create" ? t("departments.action.new") : t("departments.action.edit")}
            </h2>
            <div className="space-y-3 text-sm">
              <label className="block">
                <span className="text-muted">{t("departments.label.companyId")} *</span>
                <select
              ...
              <label className="block">
                <span className="text-muted">{t("departments.label.name")} *</span>
              ...
              <label className="block">
                <span className="text-muted">{t("departments.label.code")} *</span>
              ...
              <label className="block">
                <span className="text-muted">{t("departments.label.description")}</span>
              ...
              <label className="flex items-center gap-2">
                ...
                <span className="text-muted">{t("departments.label.active")}</span>
              </label>
            </div>
            {formMsg && <p className="mt-3 text-sm text-destructive">{formMsg}</p>}
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={closeModal} disabled={formBusy} className="rounded border border-border px-3 py-1.5 text-sm hover:bg-surface-alt disabled:opacity-40">{t("departments.action.cancel")}</button>
              <button onClick={() => void handleSubmit()} disabled={formBusy} className="rounded bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-40">
                {modal.kind === "create" ? t("departments.action.create") : t("departments.action.save")}
              </button>
            </div>
          </div>
        </div>
