...
export default function TenantAppShellPage() {
  const { me, hasCompany, navigation, navigationLoadError, tenants, tenantsLoadError } = useTenantAppSession();
  ...
  if (hasCompany === false) {
    const returnTo =
      typeof window !== "undefined"
        ...
        : "/app";
    const createCompanyHref = `/app/companies/new?returnTo=${encodeURIComponent(returnTo)}`;
    const canViewCompanies = me.privileges.includes("COMPANY_VIEW") || me.privileges.includes("COMPANY_MANAGE");
    const canCreateCompany = me.privileges.includes("COMPANY_MANAGE");

    return (
      <div className="mx-auto flex max-w-5xl flex-col gap-6" data-testid="no-company-hero">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            {canCreateCompany ? (
              <a
                ...
                data-testid="no-company-hero-cta"
              >
                Create company
              </a>
            ) : (
              <div className="rounded-md border border-border bg-background px-4 py-2 text-sm text-muted" data-testid="no-company-hero-no-access">
                You don’t have permission to create a company. Ask an admin to grant <code className="rounded bg-surface px-1">COMPANY_MANAGE</code>.
              </div>
            )}
            {canViewCompanies ? (
              <a
        ...
        <section className="rounded-xl border border-border bg-surface p-7 shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted">Follow the guided checklist below.</p>
        </section>
  ...
  const diagnosticsModal =
    state.kind === "ready" && diagnosticsOpen ? (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-5xl rounded-xl border border-border bg-surface shadow-xl">
          <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-4">
            <div className="min-w-0">
              <h2 className="truncate text-sm font-semibold text-foreground">Workspace diagnostics</h2>
              <p className="mt-0.5 text-xs text-muted">Technical details for debugging and support.</p>
            </div>
            <button
              type="button"
              className="inline-flex items-center justify-center rounded-md border border-border bg-background px-3 py-2 text-sm font-medium text-foreground hover:bg-surface-alt"
              onClick={() => setDiagnosticsOpen(false)}
              data-testid="dashboard-diagnostics-close"
            >
              Close
            </button>
          </div>
          <div className="max-h-[75vh] overflow-auto p-5">
            <div className="flex flex-col gap-6">
              ...
              {tenants.length > 1 ? (
                <section className="rounded-md border border-border bg-surface p-6 shadow-sm" data-testid="tenant-switcher">
                  <h2 className="text-sm font-medium text-foreground">Your tenants</h2>
                  <p className="mt-1 text-xs text-muted">
                    Open another tenant in the same browser session (same relay cookies).
                  </p>
              ...
              <section className="rounded-md border border-border bg-surface p-6 shadow-sm">
                ...
                <p className="mt-2 text-xs text-muted">
                  Subject export and erasure request (M1). See{" "}
                  <code className="rounded bg-background px-1">docs/modules/data-lifecycle.md</code>.
                </p>
              ...
              <section className="rounded-md border border-border bg-surface p-6 shadow-sm">
                ...
                <p className="mt-2 text-sm text-muted">
                  GET /api/bff/v1/me/navigation — filtered by privileges (sidebar mirrors this tree).
                </p>
                {navigationLoadError ? (
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-6" data-testid="tenant-app-shell">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted">Your workspace overview.</p>
        </div>
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-md border border-border bg-background px-3 py-2 text-sm font-medium text-foreground hover:bg-surface-alt"
      ...
      {state.kind === "ready" ? (
        <section className="rounded-xl border border-border bg-surface p-7 shadow-sm" data-testid="dashboard-welcome">
          <h2 className="text-sm font-semibold text-foreground">Welcome</h2>
          <p className="mt-2 text-sm text-muted">
            This dashboard is kept non-technical. Use <span className="font-medium text-foreground">Diagnostics</span> for system and integration details.
          </p>
        </section>
      ...
      {state.kind === "ready" ? (
        <div className="rounded-xl border border-border bg-surface p-7 shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted">Open the Diagnostics modal to view billing, privileges, and API navigation details.</p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              type="button"
              className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90"
              onClick={() => setDiagnosticsOpen(true)}
              data-testid="dashboard-open-diagnostics-cta"
            >
              Open diagnostics
            </button>
          </div>
