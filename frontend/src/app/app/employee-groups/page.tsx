...
export default function EmployeeGroupsPage() {
  ...
  const [confirm, setConfirm] = useState<{ item: TenantEmployeeGroupItem } | null>(null);
  ...
  async function toggleActive(item: TenantEmployeeGroupItem) {
    ...
    setBusyId(item.id);
    try {
      await patchTenantEmployeeGroupActive(item.id, true);
  ...
  async function confirmDeactivate() {
    ...
    setConfirmBusy(true);
    try {
      await patchTenantEmployeeGroupActive(item.id, false);
    ...
    <div className="mx-auto max-w-5xl space-y-6" data-testid="employee-groups-page">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <div className="flex gap-3 text-sm">
          ...
          {canManage ? (
            <Link href="/app/employee-groups/new" className="rounded bg-primary px-3 py-1 text-sm font-medium text-primary-foreground hover:opacity-90">
      ...
      {load === "loading" ? <p className="text-sm text-muted">{t("employeeGroups.state.loading")}</p> : null}
      {load === "error" ? <p className="text-sm text-destructive">{t("employeeGroups.error.load")}</p> : null}
                    ...
                    <tr key={item.id}>
                      ...
                      {canManage ? (
                        <td className="px-4 py-2 text-right">
                          <Link href={`/app/employee-groups/${item.id}/edit`} className="mr-3 text-sm text-primary underline-offset-4 hover:underline">
                            {t("employeeGroups.action.edit")}
                          </Link>
      ...
      <ConfirmDialog
        open={!!confirm}
        title="Deactivate employee group?"
