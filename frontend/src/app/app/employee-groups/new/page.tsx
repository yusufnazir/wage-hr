...
function emptyPayload(companyId = ""): TenantEmployeeGroupUpsertPayload {
  return { companyId, name: "", code: "", description: "", active: true };
}

export default function EmployeeGroupNewPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      const created = await createTenantEmployeeGroup({
        ...form,
        name: form.name.trim(),
        code: form.code.trim(),
      ...
      router.push(`/app/employee-groups/${created.id}/edit`);
    } catch {
      setError(t("employeeGroups.msg.createFailed"));
      setBusy(false);
    }
  }
  ...
  if (!canManage) {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("employeeGroups.action.new")}</h1>
  ...
  if (load === "error") {
    return (
      <div className="mx-auto max-w-lg space-y-4">
        <h1 className="text-lg font-semibold text-foreground">{t("employeeGroups.action.new")}</h1>
        <p className="text-sm text-muted">{t("employeeGroups.error.load")}</p>
      </div>
    );
  }

  return (
    <div className="mx-auto max-w-3xl space-y-6" data-testid="employee-group-form-new">
      ...
      <form onSubmit={(e) => void onSubmit(e)} className="space-y-4 rounded-md border border-border bg-surface p-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={busy} className="rounded bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">
            {t("employeeGroups.action.create")}
          </button>
