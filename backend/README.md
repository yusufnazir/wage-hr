...
- **MinIO** (optional for document presign): set **`MINIO_ENDPOINT`**, **`MINIO_ACCESS_KEY`**, **`MINIO_SECRET_KEY`**, **`MINIO_BUCKET`** (see `app.storage.minio` in `application.yml`). When any are missing, **`POST .../tenant/documents/upload-sessions`**, **`POST .../complete`**, and **`GET .../tenant/documents/{id}/download-url`** return **503**. Optional **`MINIO_VERIFY_OBJECT_BEFORE_COMPLETE=true`** runs **HEAD** on **`complete`** (default **false** for CI/local without a real object). For browser uploads from Next **`/app/documents`**, configure **CORS** on the bucket for your dev origins (e.g. `http://demo.lvh.me:3007`). **`DELETE .../tenant/documents/{id}`** soft-deletes metadata (**uploader** + **`DOCUMENT_EDIT`**); by default **`MINIO_DELETE_OBJECT_ON_SOFT_DELETE=true`** attempts **S3 delete** after soft-delete (logged failures only). Optional **`MINIO_ORPHAN_CLEANUP_ENABLED=true`** enables a **UTC cron** job to remove **orphan** keys under **`tenants/`** (no DB row, older than **`MINIO_ORPHAN_CLEANUP_MIN_OBJECT_AGE`**) and to **retry S3 delete** for old soft-deleted rows.
