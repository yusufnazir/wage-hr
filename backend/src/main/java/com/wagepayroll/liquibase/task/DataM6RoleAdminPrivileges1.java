...
public class DataM6RoleAdminPrivileges1 implements CustomTaskChange {
	...
	public void execute(Database database) throws CustomChangeException {
		...
		try {
			...
			try {
				upsertPrivilege(c, PRIV_ROLE_VIEW, "ROLE_VIEW", "View roles and their granted privileges", ts);
				upsertPrivilege(c, PRIV_ROLE_EDIT, "ROLE_EDIT", "Create roles and edit role privileges", ts);

				insertRolePrivilegeIfMissing(c, TENANT_DEMO, ROLE_ADMIN, PRIV_ROLE_VIEW);
				insertRolePrivilegeIfMissing(c, TENANT_DEMO, ROLE_ADMIN, PRIV_ROLE_EDIT);
				insertRolePrivilegeIfMissing(c, TENANT_DEMO, ROLE_VIEWER, PRIV_ROLE_VIEW);
