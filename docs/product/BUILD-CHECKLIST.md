...
| **Subscription vs gating** | An **active subscription** drives **feature flags** (`planFeatureCodes`) and plan-feature gated navigation. Authorization remains role-based privilege checks plus superadmin elevation. |
...
- [ ] **Registration + email verification (v2):** user-chosen **tenant handle**, **verify email before login**, platform **`auth.registration.default_role_template_code`** — spec [`docs/modules/account-registration.md`](../modules/account-registration.md); run `MASTER-FEATURE-END-TO-END` with that module as sole authority.
- [x] **Multi-tenant membership**; user can belong to many tenants; **roles can differ per tenant** (and later per business unit where modeled) — *`GET /api/v1/me/tenants`, seeded second tenant **`acme`** (admin = Reader, narrower role grants than `demo`); web tenant switcher on `/app`; `DataM1SecondTenantAcmeSeed1`*.
- [x] **Privileges:** action+resource catalog; role-based grants per tenant; SuperAdmin effective-all via data path (no controller shortcuts); CRUD mapped to privileges (`PRIVILEGE-MODEL`) — *`DefinedPrivilege` + `PrivilegeCatalogSyncIT`; `GET /api/v1/platform/privileges/catalog`; `GET /api/v1/tenant/privileges/pool` returns global catalog codes for editor UIs; `PermissionService` reads `platform_superadmin` for `hasPrivilege` (membership + globally registered code); `/me` `privileges` role-derived*.
...
- [x] **Tenant admin** constrained to privilege codes present in global catalog and granted to roles; tenant privilege-pool assignment endpoint is removed from API surface.
